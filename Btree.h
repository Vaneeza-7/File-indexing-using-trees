...
struct BTreeNode {
    ...
    BTreeNode(long _Degree, bool _leaf);
    ...
    void getfrom_prev(long Fid) {
        BTreeNode* child = ChildPtr[Fid];
        BTreeNode* sibling = ChildPtr[Fid - 1];
    ...
    void splitter(long i, BTreeNode* y) {
    ...
    BTreeNode* search(long numValue)
    {
        long i = 0;
...
struct BTree {
    BTreeNode* root = NULL; // Pointer to root BTreeNode 
    long Degree; // Minimum degree 
    void dlr(BTreeNode* y)
    {
        root->man(y);
    }
...
BTreeNode::BTreeNode(long t1, bool leaf1) {
    Degree = t1;
    leaf = leaf1;

    ChildPtr = new BTreeNode * [2 * Degree];
    dataObjects = new DataSet[(2 * Degree) - 1];

    N_Keys = 0;
}
void deler(BTreeNode* x)
{
    delete x;
}
void fre(BTreeNode* CocaCola)
{
    delete CocaCola;
}
